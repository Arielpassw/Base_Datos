...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['agregar_libro'])) {
    ...
    $autores_input = trim($_POST['autores']); // ahora es texto separado por comas

    // Insertar libro
    $sql = "INSERT INTO libro (titulo, isbn, anio_publicacion, ejemplares_totales, ejemplares_disponibles, estado, id_categoria)
            VALUES ($1,$2,$3,$4,$4,$5,$6) RETURNING id_libro";
    $result = pg_query_params($conn, $sql, [$titulo, $isbn, $anio, $ejemplares_totales, $estado, $id_categoria]);
    ...
    if ($result) {
        ...
        if (!empty($autores_input)) {
            ...
            foreach ($autores_array as $autor_nombre) {
                ...
                $res_check = pg_query_params($conn, "SELECT id_autor FROM autor WHERE nombre = $1", [$autor_nombre]);
                if ($res_check && pg_num_rows($res_check) > 0) {
                    $autor = pg_fetch_assoc($res_check);
                    $id_autor = $autor['id_autor'];
                } else {
                    // Insertar nuevo autor
                    $res_insert = pg_query_params($conn, "INSERT INTO autor (nombre) VALUES ($1) RETURNING id_autor", [$autor_nombre]);
                    $autor = pg_fetch_assoc($res_insert);
                    $id_autor = $autor['id_autor'];
                }

                // Relacionar libro con autor
                pg_query_params($conn, "INSERT INTO libro_autor (id_libro, id_autor) VALUES ($1,$2)", [$id_libro, $id_autor]);
            }
        }
        ...
        $msg = "Error al agregar libro: " . pg_last_error($conn);
...
if (isset($_GET['eliminar'])) {
    ...
    if ($result) {
        ...
        $msg = "Error al desactivar libro: " . pg_last_error($conn);
        $msg_class = "alert-error";
    }
}
...
<div class="container">
    ...
    <form method="POST" class="formulario">
        ...
        <label>Autores (separar varios con coma):</label>
    ...
    <table class="tabla-libros">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="editar_libro.php?id=<?= $l['id_libro'] ?>">Editar</a> |
                    <a href="?eliminar=<?= $l['id_libro'] ?>" onclick="return confirm('¿Deseas desactivar este libro?')">Eliminar</a>
                </td>
            </tr>
            <?php } ?>
        </tbody>
